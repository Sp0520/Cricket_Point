...
$st = $pdo->prepare('
    SELECT r.*, t.tournament_name, t.entry_fees, p.full_name AS player_name, tm.team_name AS team_name
...
if ($reg['payment_status'] === 'paid' || $reg['status'] === 'approved') {
    header('Location: player_dashboard.php?msg=registered');
    exit;
}

$entryFee = (float)($reg['entry_fees'] ?? 0.00);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($method, ['upi', 'card', 'netbanking'], true)) {
        ...
        if ($err === '') {
            $processing = true;
            $txId = 'CP-REG-' . strtoupper(bin2hex(random_bytes(6)));
  ...
  <div class="col-lg-8 col-xl-7">
    ...
    <?php if ($success): ?>
      ...
      <div class="cp-card p-5 text-center my-4 animate-fade-in">
        ...
        <h2 class="fw-bold mb-2">Registration Approved!</h2>
        <p class="cp-muted mb-4">Your entry fee payment has been successfully recorded and registration is approved.</p>
        ...
        <div class="p-3 mb-4 rounded bg-dark bg-opacity-50 border border-secondary text-start mx-auto" style="max-width: 400px;">
          <div class="d-flex justify-content-between mb-2">
            <span class="cp-muted small">Tournament:</span>
            <span class="small fw-semibold text-light"><?= h($reg['tournament_name']) ?></span>
          </div>
          <div class="d-flex justify-content-between mb-2">
            <span class="cp-muted small">Registrant Type:</span>
            <span class="small text-light capitalize"><?= h($reg['registrant_type']) ?></span>
          </div>
          <div class="d-flex justify-content-between mb-2">
            <span class="cp-muted small">Registrant Name:</span>
            <span class="small text-light"><?= h($reg['registrant_type'] === 'player' ? $reg['player_name'] : $reg['team_name']) ?></span>
          </div>
          <div class="d-flex justify-content-between mb-2">
            <span class="cp-muted small">Transaction ID:</span>
            <span class="small font-monospace text-light"><?= h($txId ?? '') ?></span>
          </div>
          <div class="d-flex justify-content-between mb-2">
            <span class="cp-muted small">Amount Paid:</span>
            <span class="small fw-semibold text-light">₹<?= number_format($entryFee, 2) ?> INR</span>
          </div>
          <div class="d-flex justify-content-between">
            <span class="cp-muted small">Status:</span>
            <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 rounded-pill">Paid & Approved</span>
          </div>
        </div>
        
        <a href="player_dashboard.php" class="btn btn-cp btn-lg px-5">Go to Dashboard</a>
      </div>
    ...
    <?php elseif ($processing): ?>
      ...
      <div class="cp-card p-5 text-center my-4">
        ...
        <h3 class="fw-bold mb-2">Processing Entry Fee Payment...</h3>
        <p class="cp-muted mb-0">Please do not refresh the page or click back. We are validating your transaction.</p>
    ...
    <?php else: ?>
      ...
      <div class="cp-card p-4 p-md-5 my-3">
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-4">
          <div>
            <h2 class="fw-bold mb-1">Tournament Entry Fee</h2>
            <div class="cp-muted">Complete payment to finalize your registration</div>
          </div>
        ...
        <div class="row g-4 mb-4">
          ...
          <div class="col-md-5 order-md-2">
            <div class="p-4 rounded border border-secondary" style="background: rgba(255,255,255,0.02)">
              <h5 class="fw-bold mb-3 border-bottom border-secondary pb-2">Entry Details</h5>
              <div class="small mb-1"><span class="cp-muted">Tournament:</span></div>
              <div class="fw-bold text-light mb-3"><?= h($reg['tournament_name']) ?></div>
              
              <div class="small mb-1"><span class="cp-muted">Registrant (<?= h($reg['registrant_type']) ?>):</span></div>
              <div class="text-light mb-3"><?= h($reg['registrant_type'] === 'player' ? $reg['player_name'] : $reg['team_name']) ?></div>

              <hr class="border-secondary my-3">
              <div class="d-flex justify-content-between align-items-end mb-0">
                <span class="fw-bold text-light">Entry Fee (INR)</span>
                <span class="fw-bold fs-4 text-success">₹<?= number_format($entryFee, 2) ?></span>
              </div>
            </div>
          ...
          <div class="col-md-7 order-md-1">
            <form method="post" id="checkoutForm">
              ...
              <div class="d-grid">
                <button class="btn btn-cp btn-lg py-3" type="submit">Pay ₹<?= number_format($entryFee, 2) ?> Securely</button>
              </div>
            </form>
          </div>
        </div>
